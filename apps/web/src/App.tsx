...
import { BrowserType, EnvironmentType, FailureInvestigation, ScanResult, TestCase, TestRun } from '@webtest/shared';
  ...
  const handleScanWebsite = async () => {
    ...
    try {
      const res = await fetch('/api/websites/WEB-DEMO-01/scan', {
        ...
        body: JSON.stringify({ maxPages: 10, maxDepth: 3 })
      });
      if (res.ok) {
  ...
  const handleRunTests = async () => {
    ...
    try {
      const res = await fetch('/api/websites/WEB-DEMO-01/test', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ browserType: browser })
      });
